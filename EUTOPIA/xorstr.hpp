   ...
   public:
    ...
    XORSTR_FORCEINLINE xor_string(L l, std::integral_constant<std::size_t, Size>,
        ...
        : _storage{::jm::detail::load_from_reg(
              (std::integral_constant<std::uint64_t, detail::load_xored_str8<Size>(
    ...
    XORSTR_FORCEINLINE void crypt() noexcept {
        // everything is inlined by hand because a certain compiler with a certain linker is _very_
        // slow
        ...
        ((Indices >= sizeof(_storage) / 16
              ...
              : __builtin_neon_vst1q_v(
                    ...
                    veorq_u64(__builtin_neon_vld1q_v(
                                  reinterpret_cast<const uint64_t*>(_storage) + Indices * 2, 51),
                              __builtin_neon_vld1q_v(
                                  reinterpret_cast<const uint64_t*>(keys) + Indices * 2, 51)),
                    51)),
         ...);
#else  // GCC, MSVC
        ((Indices >= sizeof(_storage) / 16
    ...
    XORSTR_FORCEINLINE pointer crypt_get() noexcept {
        // crypt() is inlined by hand because a certain compiler with a certain linker is _very_
        // slow
#if defined(__clang__)
        alignas(alignment) std::uint64_t arr[]{::jm::detail::load_from_reg(Keys)...};
        std::uint64_t* keys = (std::uint64_t*)::jm::detail::load_from_reg((std::uint64_t)arr);
