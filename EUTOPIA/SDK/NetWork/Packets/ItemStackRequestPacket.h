...
#include "../../World/Item/ItemStackNetIdVariant.h"

#include "InventoryTranscationPacket.h"
#include "Packet.h"
...
class ItemStackRequestSlotInfo {
   public:
    ...
    ItemStackRequestSlotInfo(ContainerEnumName containerEnumName, uint8_t slot,
                             ItemStackNetIdVariant networkStackID) {
        memset(this, 0, sizeof(ItemStackRequestSlotInfo));
...
class ItemStackRequestActionSwap : public ItemStackRequestActionTransferBase {
   ...
   public:
    ...
    ItemStackRequestActionSwap(const ItemStackRequestSlotInfo& s1,
                               const ItemStackRequestSlotInfo& s2) {
        // static uintptr_t vt =
        // Memory::ResolveRef(SigManager::GetSignatureAddress(SignatureEnum::ItemStackRequestAction_swap),
        // 3);
...
class ItemStackRequestActionDrop : public ItemStackRequestActionTransferBase {
   public:
    ...
    ItemStackRequestActionDrop(uint8_t amount, bool randomDropLocation,
                               const ItemStackRequestSlotInfo& slotInfo) {
        memset(this, 0x0, sizeof(ItemStackRequestActionDrop));
        using dropAction_ctor = void(__fastcall*)(ItemStackRequestActionDrop*, uint8_t, bool,
                                                  const ItemStackRequestSlotInfo&);
        // static dropAction_ctor ctor =
        // reinterpret_cast<dropAction_ctor>(Memory::ResolveRef(SigManager::GetSignatureAddress(SignatureEnum::ItemStackRequestAction_drop)));
...
struct ItemStackRequestData {
    ...
    ItemStackRequestData(std::vector<ItemStackRequestAction*> actions) {
        static uintptr_t vt = 0;  // Memory::ResolveRef(SigManager::GetSignatureAddress(SignatureEnum::ItemStackRequestData),
                                  // 3);
        this->vtable = vt;
        this->zero = 0;
